fetch(new Request("/request?q=reqassets")).then(a => {a.text().then(b=>{
    ...
    data.reqassets.forEach(element => {
        ...
        + `<tr class="Assets-Asset" id="${element["JCPS Tag"]}" onclick="selectIssue(${element['JCPS Tag']})"><td>${element['JCPS Tag']}</td><td>${element['Item Type']}</td><td>${element['Date Added']}</td><td>${element['Item Description']}</td><td>${element['Item Location']}</td></tr>`
   });
})})
